...
use crate::persistent_server_service;

#[tauri::command]
pub async fn install_available_update(app: AppHandle) -> Result<(), String> {
    let updater = app
        .updater_builder()
        .on_before_exit(persistent_server_service::prepare_persistent_server_before_exit)
        .build()
    ...
    persistent_server_service::prepare_persistent_server_for_update().await?;
    update
